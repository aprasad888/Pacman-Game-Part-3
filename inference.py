...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for prevPlace in self.legalPositions:
            gameRange = self.getPositionDistribution(self.setGhostPosition(gameState, prevPlace))
            for nextPlace, stats in gameRange.items():
                new = allPossible[nextPlace]
                # old = allPossible[prevPlace]
                prev = self.beliefs[prevPlace]
                # allPossible[prevPlace] = old * self.beliefs[]
                # allPossible[prevPlace] = old * prev
                allPossible[nextPlace] = new + (stats * prev)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while particleCount < self.numParticles:
            for place in self.legalPositions:
                if particleCount < self.numParticles:
                    self.particles.append(place)
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            allPossible = util.Counter()
            # prevDist = self.getBeliefDistribution()

            for place in self.legalPositions:
                newRange = util.manhattanDistance(place, pacmanPosition)
                eMod = emissionModel[newRange]
                allPossible[place] = (eMod * self.getBeliefDistribution()[place]) \
                                     + allPossible[place]
            ...
            if not any(allPossible.values()):
                ...
                store = []
                for extend in range(self.numParticles):
    ...
    def elapseTime(self, gameState):
        ...
        for prevPlace in self.particles:
            update = self.setGhostPosition(gameState, prevPlace)
            nextPlace = self.getPositionDistribution(update)
            store.append(util.sample(nextPlace))
    ...
    def getBeliefDistribution(self):
        ...
        spaceRange = util.Counter()

        for number in self.particles:
            spaceRange[number] = spaceRange[number] + 1

        # similar to observe method in ExactInference class
        spaceRange.normalize()
        return spaceRange
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # num = self.numGhosts
        # samplePlaces = list(itertools.product(self.legalPositions, num))
        samplePlaces = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(samplePlaces)
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for num in range(self.numGhosts):
                pos = setGhostPositions(gameState, newParticle)
                currAgent = self.ghostAgents[num]
                nextSpace = getPositionDistributionForGhost(pos, num, currAgent)
                newParticle[num] = util.sample(nextSpace)
    ...
    def getBeliefDistribution(self):
        ...
        # similar to getBeliefDistribution method in ParticleFilter class observe method in ExactInference class
        spaceRange.normalize()
        return spaceRange
