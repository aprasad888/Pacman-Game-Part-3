...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for space in livingGhostPositionDistributions:
            possibleMaxPos.append(space.argMax())

        # newLocation = None
        newStats = 0

        for counter, location in enumerate(possibleMaxPos):
            updatePos = livingGhostPositionDistributions[counter][location]
            if updatePos < newStats:
                break
        ...
        for move in legal:
            nextPlace = Actions.getSuccessor(pacmanPosition, move)
            store.append((self.distancer.getDistance(nextPlace, newLocation), move))
